...
defined( 'IWP_MIG_PLUGIN_VERSION' ) || define( 'IWP_MIG_PLUGIN_VERSION', '1.1.1' );
...
class IWP_Migration {
	...
	function render_css_for_admin_bar_btn() {
		...
		$css_rules = array(
			array(
				...
				'rules'     => array(
					'background-color' => self::get_option( 'cta_btn_bg_color', '#6b2fad' ) . ' !important',
					'color'            => self::get_option( 'cta_btn_text_color', '#fff' ) . ' !important',
				),
			),
		);
		...
		if ( ! empty( $custom_css = self::get_option( 'iwp_custom_css' ) ) ) {
			printf( '%s', $custom_css );
		}
	...
	function iwp_migration_initiate() {
		...
		if ( ! $open_link_action && ! $convert_sandbox && ! $show_domain_redirect && ! $create_ticket && empty( $expiry_hours ) ) {
			wp_send_json_error( array( 'message' => 'No migration actions are enabled. Please enable at least one option in the settings.' ) );
		}
	...
	function enqueue_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'enable_src_demo_url' => self::get_option( 'iwp_enable_src_demo_url', '' ),
			)
		);
	...
	/**
	 * Fixes the redirect URL after saving settings to include the current tab
	 * and flushes the cache to prevent stale data.
	 *
	 * @param string $location The path to redirect to.
	 * @param int    $status   Status code to use.
	 * @return string The modified redirect location.
	 */
	public function fix_settings_redirect( $location, $status ) {
		if ( isset( $_POST['option_page'] ) && $_POST['option_page'] === self::$_settings_group ) {
			// Check if settings were updated, which is part of the default redirect URL.
			if ( strpos( $location, 'settings-updated=true' ) !== false ) {
				// Aggressively clear cache to prevent stale data on reload.
				wp_cache_flush();

				// Get the tab from the hidden field in the form.
				$tab = isset( $_POST['iwp_current_tab'] ) ? sanitize_text_field( $_POST['iwp_current_tab'] ) : 'general';

				// Add the tab argument to the redirect URL.
	...
	function add_migrate_page() {
		$title_text = self::get_option( 'title_text', 'Demo Helper' );
		add_submenu_page(
	...
	function add_migrate_button( $wp_admin_bar ) {
		...
		$button_location = self::get_option( 'top_button_location', 'left' );
	...
	function render_setting_field( $field ) {
		...
		if ( $field_type === 'api_key' ) {
			...
			if ( ! empty( $field_value ) ) {
				...
				printf(
					'<input type="text" readonly style="width: 300px; background-color: #f7f7f7; color: #666;" value="%s" title="API key is masked for security - only last 4 characters shown" />',
				...
				printf(
					'<input %s type="password" style="width: 380px;" name="%s" value="" placeholder="%s" title="Enter your API key" />',
					$is_disabled,
					$field_id,
					$placeholder
				);
		...
		if ( $field_type === 'checkbox' ) {
			printf(
				'<label style="cursor: pointer;"><input type="checkbox" %s name="%s" value="yes" /> %s</label>',
		...
		if ( $field_type === 'checkbox_with_field' ) {
			...
			printf(
				'<label style="cursor: pointer; margin-bottom: 10px; display: block;"><input type="checkbox" %s name="%s" value="yes" class="iwp-checkbox-with-field" data-linked-field="%s" /> %s</label>',
				$is_checked,
				$field_id,
				$linked_field,
				$field_label
			...
			if ( ! empty( $linked_field ) ) {
				printf(
					'<input type="%s" style="width: 300px; margin-left: 20px; %s" name="%s" value="%s" placeholder="%s" class="iwp-linked-field" />',
					$input_type,
					$linked_field_style,
					$linked_field,
					$linked_field_value,
		...
		if ( $field_type === 'checkbox_with_multiple_fields' ) {
			...
			printf(
				'<label style="cursor: pointer; margin-bottom: 10px; display: block;"><input type="checkbox" %s name="%s" value="yes" class="iwp-checkbox-with-multiple-fields" data-linked-fields="%s" /> %s</label>',
				$is_checked,
				$field_id,
				esc_attr( implode( ',', $field_names ) ),
				$field_label
			);
			...
			foreach ( $linked_fields as $linked_field ) {
				...
				if ( $linked_field_type === 'checkbox' ) {
					...
					printf(
						'<label style="cursor: pointer; %s"><input type="checkbox" %s name="%s" value="yes" class="iwp-multiple-linked-field" /> %s</label>',
						$linked_field_style,
						$is_checked,
						$linked_field_name,
						esc_html( $linked_field_label )
					);
				} else {
					...
					printf(
						'<input type="%s" style="%s" name="%s" value="%s" placeholder="%s" class="iwp-multiple-linked-field" />',
						$linked_field_type,
						$linked_field_style,
						$linked_field_name,
						esc_attr( $linked_field_value ),
						esc_attr( $linked_field_placeholder )
					);
				}
				printf( '</div>' );
			}
			...
			<script>
				(function ($) {
					$(document).ready(function () {
						...
						wp.codeEditor.initialize($('#<?php echo $random_id; ?>'), editorSettings);
					});
				})(jQuery);
			</script>
			<?php
		...
		if ( $field_type === 'radio' ) {
			...
			if ( ! empty( $options ) ) {
				...
				foreach ( $options as $value => $label ) {
					printf(
						'<label style="padding-right: 10px;"><input type="radio" name="%s" value="%s" %s /> %s</label>',
						$field_id,
						esc_attr( $value ),
	...
	public static function get_option( $option_name, $default = '' ) {
		...
		if ( empty( $option_value ) ) {
			...
			$this_option = $all_options[ $option_name ] ?? array();
